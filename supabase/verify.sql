...
do $$ declare a uuid; b uuid; michael public.players; safwat public.players; tid uuid; cid uuid; bid uuid; n int; denied bool; begin
 ...
 insert into public.reservations(type,team_id,date,start_time,end_time,location,created_by) values('practice',a,'2099-01-01','18:00','19:00','Lakewood Elementary',auth.uid()) returning id into bid;
 denied=false;begin insert into public.reservations(type,team_id,date,start_time,end_time,location,created_by) values('practice',b,'2099-01-01','18:00','19:00','Lakewood Elementary',auth.uid());exception when insufficient_privilege then denied=true;end;
 if not denied then raise exception 'FAIL: other-team booking';end if;
 denied=false;begin insert into public.reservations(type,home_team_id,away_team_id,date,start_time,end_time,location,created_by) values('game',a,a,'2099-01-01','18:00','19:00','Lakewood Elementary',auth.uid());exception when check_violation then denied=true;end;
 if not denied then raise exception 'FAIL: same-team game';end if;
 insert into public.reservations(type,date,start_time,end_time,location,created_by) values('friendly','2099-01-01','18:00','19:00','Scheels',auth.uid());
 denied=false;begin insert into public.reservations(type,date,start_time,end_time,location,created_by) values('friendly','2099-01-01','17:59','19:00','Scheels',auth.uid());exception when check_violation then denied=true;end;
 if not denied then raise exception 'FAIL: before-six booking';end if;
 denied=false;begin insert into public.reservations(type,date,start_time,end_time,location,created_by) values('friendly','2099-01-01','18:00','19:00','Other field',auth.uid());exception when check_violation then denied=true;end;
 if not denied then raise exception 'FAIL: invalid location';end if;
